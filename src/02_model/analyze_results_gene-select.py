...
# Load checkpoint first to get num_genes
checkpoint = torch.load("checkpoints/best_model.pt", weights_only=False)
num_genes_in_checkpoint = len(checkpoint['gene_names'])

print(f"Checkpoint was trained on {num_genes_in_checkpoint} genes")

# Create dataloaders with SAME number of genes as checkpoint
train_loader, val_loader, num_genes, gene_names, _ = create_dataloaders(
  config, 
  top_n_genes=num_genes_in_checkpoint  # <-- MATCH checkpoint
)

# Now model will match
...
axes[0].set_title(f'Distribution of per-gene correlations (n={len(gene_corrs)} genes)')
axes[0].legend()
...
plt.tight_layout()
plt.savefig('gene_analysis_spatial_top5k.png', dpi=150, bbox_inches='tight')
print("\nSaved: gene_analysis_spatial_top5k.png")
